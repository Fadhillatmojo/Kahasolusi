...
<table class="table rounded-corner">
    ...
    <tbody>
        ...
        <tr>
            <td class="align-middle text-center index" style="font-weight: bold">{{ $testimonial->testimonial_client }}
            </td>
            ...
            <td class="align-middle text-center index">
                ...
                <img src="{{ $testimonial->testimonial_image_url }}" alt="{{ $testimonial->testimonial_image_url }}"
                    width="150" height="150">
                @else
                <img src="{{ (  asset('storage/testimonials/' . $testimonial->testimonial_image_url)) }}"
                    alt="{{ $testimonial->testimonial_image_url }}" width="150" height="150">
                @endif
            </td>
            <td class="text-center align-middle none-border-right index">
                <div class="d-flex justify-content-center">
                    <a href="{{ route('testimonials.edit', $testimonial->testimonial_id) }}"><img
                            src="{{ asset('../adminassets/img/global/action/iconActionEdit.svg') }}"></a>
                    <form id="deleteForm_{{ $testimonial->testimonial_id }}"
                        action="{{ route('testimonials.destroy', $testimonial->testimonial_id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="button" id="btn-delete"
                            onclick="showDeleteConfirmation('{{ $testimonial->testimonial_id }}')"><img
