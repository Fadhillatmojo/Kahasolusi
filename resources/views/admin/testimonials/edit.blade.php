...
<form method="POST" action="{{ route('testimonials.update', $testimonial->testimonial_id) }}"
	...
	<table id="table-form" class="w-100">
		<tr>
			...
			<td>
				<input type="text" class="form-control @error('testimonial_client') is invalid @enderror"
					...
					value="{{ $testimonial->testimonial_client }}">
		...
		<tr>
			...
			<td>
				<textarea class="form-control" placeholder="Description" id="testimonial_desc"
					name="testimonial_desc" rows="5">{{ $testimonial->testimonial_desc }}</textarea>
		...
		<tr>
			...
			<td>
				<div id="img-container" class="d-inline-block align-items-center py-2 px-3">
					...
					<img id="preview-image" src="{{ $testimonial->testimonial_image_url }}" class="mb-2"
						alt="{{ $testimonial->testimonial_image_url }}" width="150" height="150">
					@else
					<img id="preview-image"
						src="{{ (  asset('storage/testimonials/' . $testimonial->testimonial_image_url)) }}"
						alt="{{ $testimonial->testimonial_image_url }}" width="150" height="150">
					@endif
					...
					<input id="img-upload" type="file"
						class="d-none @error('testimonial_image_url') is-invalid @enderror"
