    ...
    <form method="POST" action="{{ route('founders.update', $founder->person_id) }}" enctype="multipart/form-data">
		...
		<table id="table-form" class="w-100">
			<tr>
				...
				<td>
				    <input type="text" class="form-control @error('person_name') is invalid @enderror" id="person_name" name="person_name" placeholder="Name" value="{{ $founder->person_name }}">
			...
			<tr>
				...
				<td>
				    <input type="text" class="form-control @error('person_position') is invalid @enderror" id="person_position" name="person_position" placeholder="Position" value="{{ $founder->person_position }}">
				...
				<td>
					<div id="img-container" class="d-inline-block align-items-center py-2 px-3">
						@if (Str::contains($founder->person_image_url, ['http', 'https']))
							<img id="preview-image" src="{{ $founder->person_image_url }}" class="mb-2" alt="{{ $founder->person_image_url }}" width="150" height="150">
						@else
							<img id="preview-image" src="{{ (  asset('storage/founders/' . $founder->person_image_url)) }}" alt="{{ $founder->person_image_url }}" width="150" height="150">
						@endif
						<label id="label-upload" for="img-upload" class="ms-3">
							<img src="{{ asset('../adminassets/img/global/action/iconUpload.svg') }}">
						</label>
						<input id="img-upload" type="file" class="d-none @error('person_image_url') is-invalid @enderror" name="person_image_url" onchange="document.getElementById('preview-image').src = window.URL.createObjectURL(this.files[0])">
